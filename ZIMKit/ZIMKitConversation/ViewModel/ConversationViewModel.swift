...
extension ConversationViewModel {
    func loadConversations(_ callback: ((ZIMError) -> Void)? = nil) {
        ...
        ZIMKit.zim?.queryConversationList(with: quryConfig, callback: { [weak self] zimConversations, error in
    ...
    func clearConversationUnreadMessageCount(_ conversationID: String, type: ZIMConversationType) {
        ZIMKit.zim?.clearConversationUnreadMessageCount(conversationID, conversationType: type, callback: { _, _, _ in
    ...
    func deleteConversation(at index: Int, callback: ((ZIMError) -> Void)?) {
        ...
        ZIMKit.zim?.deleteConversation(model.conversationID, conversationType: model.type, config: config, callback: { _, _, error in
            guard let callback = callback else { return }
            callback(error)
        })
    }
