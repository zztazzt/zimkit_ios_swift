...
extension MessagesViewModel {
    ...
    func queryHistoryMessage(by type: ZIMConversationType, callback: queryHistoryCallback? = nil) {
        ...
        ZIMKit.zim?.queryHistoryMessage(
    ...
    func queryGroupMemberList(_ callback: ((ZIMError) -> Void)? = nil) {
        ...
        ZIMKit.zim?.queryGroupMemberList(
    ...
    func queryGroupInfo(_ callback: ((ZIMGroupFullInfo?, ZIMError) -> Void)? = nil) {
        ZIMKit.zim?.queryGroupInfo(byGroupID: conversationID, callback: { info, error in
            callback?(info, error)
        })
    }
    ...
    func sendTextMessage(_ message: Message, callback: ((ZIMError) -> Void)? = nil) {
        ...
        message.senderUserID = ZIMKit.userInfo?.userID ?? ""
    ...
    func clearConversationUnreadMessageCount() {
        ZIMKit.zim?.clearConversationUnreadMessageCount(conversationID, conversationType: conversationType, callback: { _, _, _ in
    ...
    func downloadMediaMessage(_ message: MediaMessage, callback: ((ZIMError) -> Void)? = nil) {
        ...
        ZIMKit.zim?.downloadMediaFile(with: zimMsg, fileType: .originalFile, progress: { _, _, _ in
    ...
    private func updateMessageUserInfo(with message: Message) {
        if message.conversationType == .group {
            ...
            if message.direction == .send && member == nil {
                message.senderUsername = ZIMKit.userInfo?.userName
                message.senderUserAvatar = ZIMKit.userInfo?.userAvatarUrl
            }
